# 📚 DevOps Resources for Students

This file lists **free and high-quality resources** to help students learn DevOps concepts, tools, and workflows.

---

## 🌐 Official Documentation

- **Git**
  - [Git Documentation](https://git-scm.com/doc) – Official guide and reference
- **GitHub**
  - [GitHub Docs](https://docs.github.com/en) – Official tutorials and guides
- **Docker**
  - [Docker Docs](https://docs.docker.com/) – Official documentation and tutorials
- **Kubernetes**
  - [Kubernetes Docs](https://kubernetes.io/docs/) – Official Kubernetes guides
- **Prometheus**
  - [Prometheus Documentation](https://prometheus.io/docs/introduction/overview/)
- **Grafana**
  - [Grafana Docs](https://grafana.com/docs/grafana/latest/)

---

## 🎓 Free Online Courses

- **Git & GitHub**
  - [Try Git (interactive)](https://try.github.io/)  
  - [Introduction to Git and GitHub - Coursera](https://www.coursera.org/learn/introduction-git-github)
- **Docker**
  - [Docker for Beginners - freeCodeCamp](https://www.youtube.com/watch?v=3c-iBn73dDE)
- **CI/CD**
  - [GitHub Actions Tutorial - freeCodeCamp](https://www.youtube.com/watch?v=R8_veQiYBjI)
- **Kubernetes**
  - [Kubernetes Basics - Kubernetes Official](https://kubernetes.io/docs/tutorials/kubernetes-basics/)
- **Monitoring**
  - [Prometheus & Grafana - freeCodeCamp](https://www.youtube.com/watch?v=2D52X0emlJo)

---

## 📘 Books (Free / Open Source)

- [The DevOps Handbook (summary & notes)](https://itrevolution.com/book/the-devops-handbook/)
- [Docker Cheat Sheet](https://dockerlabs.collabnix.com/docker/cheatsheet/)
- [Kubernetes Cheat Sheet](https://kubernetes.io/docs/reference/kubectl/cheatsheet/)
- [Prometheus: Up & Running (O’Reilly)](https://www.oreilly.com/library/view/prometheus-up/9781492034140/)

---

## 📺 YouTube Channels

- [freeCodeCamp.org](https://www.youtube.com/@freecodecamp) – Full courses on Git, Docker, CI/CD, K8s
- [TechWorld with Nana](https://www.youtube.com/@TechWorldwithNana) – DevOps tutorials with diagrams
- [The Net Ninja](https://www.youtube.com/@TheNetNinja) – Git, Docker, CI/CD series

---

## 📝 Study Tips

- Follow the **roadmap folder** in this repo step by step  
- Practice with **hands-on labs** and playgrounds  
- Use **cheat sheets** for commands (Git, Docker, K8s, Prometheus)  
- Join **DevOps communities**: Slack, Discord, Reddit, StackOverflow

---

## 🤝 Contribution

Feel free to **add more resources** via Pull Request or suggest in Issues.  
Help make this guide more useful for other students!  

---
